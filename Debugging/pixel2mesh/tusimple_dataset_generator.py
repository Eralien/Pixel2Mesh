...
def plot_line(height_num, width_num, arr_width, arr_height,
              binaryimage, instanceimage, image, lane_hist=np.array([20, 20, 20])):
    lane_hist_ = lane_hist
            ...
            if arr_width[j][i-1] > 0 and arr_width[j][i] > 0:
                ...
                if i > 0:
                    ...
                    cv2.line(instanceimage, (int(arr_width[j][i-1]), int(arr_height[i-1])), (int(
                        arr_width[j][i]), int(arr_height[i])), lane_hist_, 10)
...
def plot_circle(height_num, width_num, arr_width, arr_height,
                ...
                color_dist=np.array([[0, 0, 1], [0, 1, 0], [1, 0, 0],
                                     [0, 1, 1], [1, 1, 0], [1, 0, 1]])*255):
    for j in range(width_num):
        if j < 6:
            color_lanedot = color_dist[j, :]
        for i in range(height_num):
            if arr_width[j][i] > 0:
                if j < 6:
...
def dataset_gen(tusimple_path_base, train_flag=True,
    ...
    for line in file.readlines():
        ...
        # for i in range(height_num):
        #     lane_hist = np.array([20, 20, 20])
        #     for j in range(width_num):
        #         if arr_width[j][i-1] > 0 and arr_width[j][i] > 0:
        #             binaryimage[int(arr_height[i]),int(arr_width[j][i])] = [255,255,255]
        #             instanceimage[int(arr_height[i]),int(arr_width[j][i])] = lane_hist

        #             if arr_width[j-1][i] > 0 and j>0:
        #                 cv2.line(binaryimage, (int(arr_width[j-1][i]),int(arr_height[i])), (int(arr_width[j][i]),int(arr_height[i])), (100,100,100), 1)

        #             if i>0:
        #                 cv2.line(binaryimage, (int(arr_width[j][i-1]),int(arr_height[i-1])), (int(arr_width[j][i]),int(arr_height[i])), np.array([255,255,255])/4, 10)
        #                 cv2.line(instanceimage,(int(arr_width[j][i-1]),int(arr_height[i-1])), (int(arr_width[j][i]),int(arr_height[i])), lane_hist, 10)
        #                 cv2.line(image,(int(arr_width[j][i-1]),int(arr_height[i-1])), (int(arr_width[j][i]),int(arr_height[i])), lane_hist, 10)
        #         lane_hist += 50

        # For color assign to each dot, makes the plot easier to analyse
